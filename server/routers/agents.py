...
class RegisterRequest(BaseModel):
    ...
    def sanitize_hostname(cls, v: str) -> str:
        # Only allow hostname-safe characters
...
async def register_agent(
    ...
    """Register agent or return existing registration (idempotent)."""
    client_ip = request.headers.get("X-Forwarded-For", "").split(",")[0].strip() or (
        request.client.host if request.client else None
    )

    # Look up existing machine
    ...
    if machine:
        ...
        result = await db.execute(
            select(AgentToken).where(
                AgentToken.machine_id == machine.id,
        ...
        if not agent_token:
            ...
            # Return a new token (re-registration)
            raw_token, token_hash = generate_agent_token()
            agent_token.token_hash = token_hash

        await db.commit()
        logger.info("agent_re_registered", machine_id=machine.id, hostname=machine.hostname)
        return RegisterResponse(
            token=raw_token,
            machine_id=machine.id,
            message="Machine re-registered successfully",
        )
    ...
    logger.info("agent_registered", machine_id=machine.id, hostname=machine.hostname, os=payload.os_type)
    return RegisterResponse(
        token=raw_token,
        machine_id=machine.id,
...
async def submit_heartbeat(
    ...
    """Submit agent heartbeat with current machine state."""
    ...
    # Calculate energy for this heartbeat interval
    ...
    return HeartbeatResponse(
        ...
        energy_wasted_kwh=machine.energy_wasted_kwh,
    )
